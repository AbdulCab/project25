...
get('/pokedex/:name') do
  ...
  types.each do |type|
    effectiveness_data = db.execute <<-SQL, [type]
      SELECT atk.name AS attacking_type, te.effectiveness
      FROM type_effectiveness te
      JOIN types atk ON te.attacking_type_id = atk.id
      JOIN types def ON te.defending_type_id = def.id
      WHERE def.name = ?
    SQL
  ...
  @evolution_chain = db.execute <<-SQL, [@pokemon["id"], @pokemon["id"], @pokemon["id"]]
    SELECT p.id, p.name, e.evolution_condition
    FROM evolution_chart e
    JOIN Pokemons p ON e.pokemon_id = p.id OR e.pre_evolution_id = p.id
    WHERE (e.pre_evolution_id = ? OR e.pokemon_id = ?) AND p.id != ?
  SQL
